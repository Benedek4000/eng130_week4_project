...
def record(out):
    ...
    while(rec):
        time.sleep(0.045)
        # test_frame.append(rec_frame)
        out.write(rec_frame)
    # se = Thread(target = test_join, args = [test_frame,])
    # se.start()

def test_join(test_frame):
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    
    out = cv2.VideoWriter('./in/second.avi', fourcc, 20, (640, 480))

    for pic in test_frame:
        # time.sleep(0.045)
        out.write(pic)
    out.release()
    print("done second video--------------------")
